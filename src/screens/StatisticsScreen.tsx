...
export const StatisticsScreen: React.FC<StatisticsScreenProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col px-4 pt-3 pb-4 max-w-sm mx-auto w-full overflow-y-auto space-y-4 select-none">
      ...
      {/* Waveform Card */}
      <div className="p-3.5 rounded-2xl bg-zt-surface border border-zt-border space-y-2 shadow-sm">
        <div className="flex items-center justify-between text-xs">
          <span className="font-semibold text-zt-text">Throughput</span>
          <div className="flex items-center gap-3 text-[11px] font-mono">
            <span className="flex items-center gap-1 text-emerald-500">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              {formatSpeed(trafficStats.downloadSpeed)}
            </span>
            <span className="flex items-center gap-1 text-zt-accent">
              <span className="w-1.5 h-1.5 rounded-full bg-zt-accent" />
        ...
        {/* SVG Waveform */}
        <div className="h-20 w-full relative bg-zt-surface-2 rounded-xl p-1 overflow-hidden">
          <svg className="w-full h-full" viewBox="0 0 300 70" preserveAspectRatio="none">
            <line x1="0" y1="20" x2="300" y2="20" stroke="rgba(148, 163, 184, 0.2)" strokeDasharray="3 3" />
            <line x1="0" y1="45" x2="300" y2="45" stroke="rgba(148, 163, 184, 0.2)" strokeDasharray="3 3" />
            <polyline
              fill="none"
              stroke="#35C77B"
              strokeWidth="2"
      ...
      <div>
        ...
        <div className="rounded-2xl bg-zt-surface border border-zt-border p-3 text-xs space-y-2 font-mono shadow-sm">
          <div className="flex justify-between">
            <span className="text-zt-text-faint">ENGINE</span>
            <span className="text-zt-text font-medium">Xray-Core + TUN L3</span>
          </div>
          <div className="flex justify-between">
            <span className="text-zt-text-faint">VIRTUAL IP</span>
            <span className="text-zt-text font-medium">10.233.233.2</span>
          </div>
          <div className="flex justify-between">
            <span className="text-zt-text-faint">ROUTING</span>
            <span className="text-zt-text font-medium">Split 0.0.0.0/1, 128.0.0.0/1</span>
          </div>
        </div>
      </div>
    </div>
